...
However, this will never end and in a few seconds a StackOverflow Exception is thrown.
 In order for the recursion to stop it has to have a “bottom”. The bottom of the recursion is GetFibonacci(2)
...
For the Nth Fibonacci number, we calculate the N-1th and the N-2th number,
 but for the calculation of N-1th number we calculate the N-1-1th(N-2th) and the N-1-2th number,
...
public class FibonachiRecursive {
    public static void main(String[] args) {
        ...
        int lastIndexNumbers = 0;
        int numberIndex = Integer.parseInt(scanner.nextLine());
        int [] fibonachiNumbers = new int[numberIndex+2];
        ...
        lastIndexNumbers += fibonachiNumbers[numberIndex];
        System.out.print(lastIndexNumbers);
    }
}